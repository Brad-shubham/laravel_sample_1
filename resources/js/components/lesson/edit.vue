            ...
            <div class="row add-course">
                <div class="col-md-9">
                    <form enctype="multipart/form-data">
                        <div class="common-border px-3 py-0">
                            <div class="form-row">
                                ...
                                <div class="form-group col-md-6">
                                    ...
                                    <select class="custom-select" name="book-id" id="book-id"
                                            :disabled="lesson.disable_assignment"
                            ...
                            <draggable v-model="form.paragraphs" class="tb-lesson" @start="drag=true" @end="drag=false">
                ...
                <div class="col-md-3">
                    <div class="submit-row list-group ">
                        ...
                        <button type="submit" @click="showDeleteLessonPopup()" title="delete"
...
export default {
    ...
    data() {
        return {
            form: new Form(this.lesson, {resetOnSuccess: false}),
            action: route('lessons.update', this.lesson.id),
    ...
    created() {
        this.form.paragraphs.forEach((paragraph) => {
            if (paragraph.image != null) {
                paragraph.show_paragraph_image = true;
            } else {
                paragraph.image_name = 'Choose Image';
            }
        });
    },
    mounted() {
    ...
    methods: {
        ...
        deleteLesson() {
            this.isLoading = true;
            this.$http.delete(route('lessons.destroy', this.lesson.id))
                .then(response => {
                ...
                .catch(error => {
                    this.isLoading = false;
                    this.$toasted.error('Unable to delete lesson.');
                })
        },
        remove() {
            if (this.selectedParagraphIndex === null && this.selectedParagraphQuestionIndex === null) {
                this.deleteLesson();
            } else if (this.selectedParagraphQuestionIndex === null) {
                this.isLoading = true;

                let paragraphId = this.form.paragraphs[this.selectedParagraphIndex].id;
                this.form.paragraphs.splice(this.selectedParagraphIndex, 1);

                if (typeof paragraphId !== 'undefined') {
                    this.$http.delete(route('paragraphs.destroy', paragraphId))
                        .then(response => {
                            if (response.data.status === true) {
                                this.isLoading = false;
                                this.$toasted.success(response.data.message);
                        ...
                        .catch(error => {
                            this.isLoading = false;
                            console.log(error);
                            this.$toasted.error('Unable to delete the paragraph.');
                        })
                } else {
                    this.isLoading = false;
                ...
                let paragraphQuestionId = this.form.paragraphs[this.selectedParagraphIndex].questions[this.selectedParagraphQuestionIndex].id;
                this.form.paragraphs[this.selectedParagraphIndex].questions.splice(this.selectedParagraphQuestionIndex, 1);

                if (typeof paragraphQuestionId !== 'undefined') {
                    this.$http.delete(route('paragraphQuestions.destroy', paragraphQuestionId))
                        .then(response => {
                            if (response.data.status === true) {
                                this.isLoading = false;
                                this.$toasted.success(response.data.message);
                        ...
                        .catch(error => {
                            this.isLoading = false;
                            this.$toasted.error('Unable to delete the paragraph question.');
                        })
                } else {
                    this.isLoading = false;
