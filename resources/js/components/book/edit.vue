            ...
            <div class="row add-course">
                ...
                <div class="col-md-3">
                    <div class="submit-row list-group ">
                        ...
                        <button type="submit" @click="showDeleteBookModal()" title="Delete"
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                form: new Form({...this.book, extension: null}, {resetOnSuccess: false}),
                action: route('books.update', this.book.id),
                showDeletePopup: false,
                bookSectionIndex: null,
                selectedBookSectionIndex: null,
                message: '',
                isLoading: false,
                isFullPage: true,
                formData: null,
        ...
        created() {
            if (this.form.cover_image != null) {
                this.showCoverImage = true;
            } else {
                this.form.cover_image_name = 'Choose Image';
            }

            this.form.book_sections.forEach((book_section) => {
                if (book_section.image != null) {
                    book_section.show_image = true;
                } else {
                    book_section.image_name = 'Choose Image';
                }
            });
        },
        mounted() {
        ...
        methods: {
            ...
            remove() {
                if (this.selectedBookSectionIndex === null) {
                    ...
                    let bookSectionId = this.form.book_sections[this.selectedBookSectionIndex].id;
                    this.form.book_sections.splice(this.selectedBookSectionIndex, 1);

                    if( typeof bookSectionId !== 'undefined'){
                        this.$http.delete(route('bookSections.destroy', bookSectionId))
                            .then(response => {
                                if (response.data.status === true) {
                                    this.isLoading = false;
                                    this.$toasted.success(response.data.message);
                            ...
                            .catch(error => {
                                this.isLoading = false;
                                this.$toasted.error('Unable to delete the book section.');
                            })
                    }else{
